...
if __name__ == '__main__':
    while True:
        ...
        match s:
            ...
            case 'help':
                print('''commands:
  check <(int)offset>    查看offset位置的武将
  random [<(int)count>]  直接随机为count个玩家分配武将
  draw <(int)count>      选出count个武将
  list                   列出所有武将
  help                   查看此帮助
  exit                   退出程序''')
                continue
        ...
        if s.startswith('check '):
            ...
            if i > len(allChampions):
                ...
                smp = random.sample(allChampions, i)
                random.shuffle(smp)
                for j in range(len(smp)):
                    print(j, '号位：', smp[j], sep='')
            ...
            if i >= len(allChampions):
                ...
                print("选取", i, "个武将：", sep='')
                for c in random.sample(allChampions, i):
                    print(c, end='\n')
        else:
            print("未知指令。")
